#!/usr/bin/env python3
"""
FIXED FINAL REFINED NLP MODULE FOR SINGLE TARGET POKEMON IDENTIFICATION
======================================================================
...
class SingleTargetDataset(Dataset):
    ...
    def __init__(self, data: List[Dict], tokenizer, max_length: int = 1024):
        # Only use positive samples (no negative samples for now to avoid stratification issues)
        self.data = [item for item in data if 'label' in item and item['label'] != -1]
...
def train_single_target_model(output_dir: str = './final_pokemon_nlp') -> Tuple[nn.Module, AutoTokenizer]:
    ...
    # FIXED: Extract labels properly for stratification
    # Only use samples with valid labels (exclude any potential negative samples)
    valid_samples = [item for item in training_data if item['label'] != -1 and item['label'] is not None]
    labels_for_stratify = [item['label'] for item in valid_samples]
    
    logger.info(f"Valid samples for training: {len(valid_samples)}")
    logger.info(f"Labels for stratification: {len(labels_for_stratify)}")
